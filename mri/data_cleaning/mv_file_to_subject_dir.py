...
import pandas as pd 
#%%
participant_file_path = r'/mnt/data/Project/DCM/participants.xlsx'
...
origin_file_dir = r'/mnt/data/Project/DCM/tmp/2021.12.20train_data/total_test'
origin_file_list = os.listdir(origin_file_dir)

copy_count = 0
have_count = 0
for sub in subjects[6:28]:
    ...
    for ori_file in origin_file_list:
        ...
        if (exp_id.upper() == file_id) or (exp_id.lower() == file_id):
            ...
            tar_path = os.path.join(data_dir,sub,'Behaviour/total_test',ori_file)
...
#%%
# 将符合标准的MEG文件移动到BIDS/derivatives/behaviour/data 目录下
import os 
...
for subj in subjects:
    ...
    for file in meg_tmp_list:
        ...
        if ('.csv' in file) and (file_size>12288):
            if ('loop' not in file) and ('trial' not in file) and (('block' not in file)):
    ...
    for file in meg_data_list:
        ...
        mv_file_num += 1
    print(subj,"copy completed.",'Move file:',mv_file_num)