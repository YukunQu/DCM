...
def copy_files(src_base_dir, dest_base_dir, sub_list, keyword=False):
    ...
    for sub in tqdm(sub_list, desc="Copying files"):
        print(sub,"started!")
        src_dir = os.path.join(src_base_dir, sub, "Behaviour/meg_task-1DInfer")
        dest_dir = os.path.join(dest_base_dir, sub, "Behaviour/meg_task-1DInfer")
...
dest_base_dir = "/mnt/data/tmp/ToLuoYao/Behaviour"

participants_tsv = r'/mnt/workdir/DCM/BIDS/participants.tsv'
participants_data = pd.read_csv(participants_tsv, sep='\t')
data = participants_data.query(f'(meg_neuro>=0.2)')
sub_list = [s.replace("-",'_') for s in data['Participant_ID'].tolist()][3:]
