...
def game2Test(test_df,mapDir):
    ...
    # sample meg pairs
    sampleAngleSet = uniformSampling(test_df,5)
    sampleAngleSet = sampleAngleSet.sample(frac=1)
    fightRule = ['1A2D']*30 + ['1D2A']*30
    sampleAngleSet['fightRule'] = fightRule
    sampleAngleSet = sampleAngleSet.sample(frac=1)
    sampleAngleSet['j1'] = [round(j,1) for j in np.random.random(60) * (3-1) + 1]
    sampleAngleSet['j2'] = [round(j,1) for j in np.random.random(60) * (3-1) + 1]
    sampleAngleSet['j3'] = [round(j,1) for j in np.random.random(60) * (3-1) + 1]
    game2test1 = sampleAngleSet[:30]
    game2test2 = sampleAngleSet[30:]
...
if __name__ == '__main__':
    levels=5
    setNum=10
    newMonsterPoints = {'nm1':(2.5,2.5),'nm2':(2.5,3.5),
                    'nm3':(3.5,2.5),'nm4':(3.5,3.5)}
    taskDir = r'C:\Myfile\File\工作\PhD\Development cognitive map\experiment\task\dcm_day4\fMRI\game2'
