...
data = participants_data.query(f'(game1_fmri>=0.5)and(Age>18)')
...
for subj in subjects:
    sub_res = pd.read_csv(f'/mnt/data/DCM/derivatives/peri_event_analysis/dmPFC/sub-{subj}_peri_event_analysis.csv')
...
sns.lineplot(x='time_point', y='distance_z', data=results2, ax=ax,errorbar='se',label='distance')
sns.lineplot(x='time_point', y='value_z', data=results, ax=ax,errorbar='se',label='value')
ax.legend()
# set y title
ax.set_ylabel('beta')
