...
noddi_file_dir = r'/mnt/data/DCM/HNY/Diffusion/NODDI/Organized'
for noddi_metrics_name in os.listdir(noddi_file_dir):
    # load subject info
    ...
    # load vbm file
    noddi_metrics = pd.read_csv(os.path.join(noddi_file_dir, noddi_metrics_name))
    noddi_metrics = noddi_metrics.drop('Group', axis=1)

    # update sub_id for vbm meteric
    sub_list = noddi_metrics['Subject']
    sub_id_list = []
    for sub in sub_list:
        sub_id = sub.split("_")[3]
        sub_id_list.append('sub-' + sub_id)

    noddi_metrics['Subject'] = sub_id_list
    noddi_metrics = noddi_metrics.sort_values('Subject')
    ...
    print(noddi_metrics_name, ":Those validate subjects not have diffusion files:", d1)
    for s in sub_id_list:
        if s not in val_sub:d2.append(s)
    print(noddi_metrics_name, ":The subjects with diffusion files are not validated:", d2)
    diff = d1+d2
    ...
    noddi_metrics = noddi_metrics[~noddi_metrics['Subject'].isin(diff)].sort_values('Subject')

    val_sub = val_sub_info['Participant_ID'].to_list()
    sub_id_list = noddi_metrics['Subject'].to_list()
    ...
    metrics_names = list(noddi_metrics.columns)

    metrics_names.remove("Subject")
    result = {}
    for km in metrics_names:
        dki_metric = noddi_metrics[km]
    ...
    for key, value in result.items():
        ...
        if p < (0.05/(len(result))):
            ...
            save_path = os.path.join('/mnt/workdir/DCM/result/structure/DKI/report', noddi_metrics_name.replace('csv', 'txt'))
            with open(save_path, 'a') as f:
                f.write(key + " : " + "\n" + 'r:' + str(round(r, 3)).zfill(3) + '   ' + 'p:' + str(round(value[1], 4)) + "\n")

    save_path = os.path.join('/mnt/workdir/DCM/result/structure/NODDI', noddi_metrics_name.replace('csv', 'json'))
