...
class Game2EVonM2(object):
    ...
    def game2ev_hexonM2(self, ifold):
        self.starttime = self.cal_start_time()
        m1ev = self.genM1ev()
        deev = self.genDeev()
        trial_corr, accuracy = self.label_trial_corr()
        m2ev_corr, m2ev_error = self.genM2ev(trial_corr, True)
        pmod_sin, pmod_cos = self.M2pm(m2ev_corr, ifold)
