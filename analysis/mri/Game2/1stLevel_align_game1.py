...
def game2_align_game1_1stLevel(subject_list, set_id, runs, ifold, configs):
    # start cue
    print("set", set_id, " ", ifold, " start!")
    ...
    templates = {'func': pjoin(data_root, 'sub-{subj_id}/func',
                 ...
                 'event': pjoin(event_dir, 'sub-{subj_id}', 'game2', 'alignPhi', ROI, ifold,
                                'sub-{subj_id}_task-game2_run-{run_id}_events.tsv'),  # look out
    ...
    working_dir =  '/mnt/workdir/DCM/BIDS/derivatives/Nipype/working_dir/game2/alignPhi/{}' \
                  'Set{}/{}'.format(ROI, set_id, ifold)
    container_path = os.path.join('game2','alignGame1', ROI, f'Set{set_id}')
    ...
    cont03 = ['alignPhi',                    'T', condition_names, [1, 1]]
...
if __name__ == "__main__":
    ...
    configs = {'data_root': r'/mnt/workdir/DCM/BIDS/derivatives/fmriprep_volume_ica',
               ...
               'ROI':'vmpfc-individual'}

    sets = {'all':[1,2]}

    folds = range(6,7)  # look out

    for set_id, runs in sets.items():
        for i in folds:
            ifold = str(i) + 'fold'
            game2_align_game1_1stLevel(subject_list, set_id, runs, ifold, configs)

            cmap_dir = f'/mnt/workdir/DCM/BIDS/derivatives/Nipype/game2/alignGame1/{configs["ROI"]}/Set{set_id}'
