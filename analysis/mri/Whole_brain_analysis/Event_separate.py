...
class Game2EV(object):
    ...
    def pressButton(self):
        if self.dformat == 'trial_by_trial':
            onset = self.behData['dResp.started'] - self.starttime
            duration = 0
            angle = self.behData['angles']
            pbev = pd.DataFrame({'onset':onset,'duration':duration,'angle':angle})
            pbev['trial_type'] = 'pressButton'
            ...
            onset = self.behData['dResp.started_raw'] - self.starttime
            duration = 0
            angle = self.behData['angles']
            pbev = pd.DataFrame({'onset':onset,'duration':duration,'angle':angle})
            pbev['trial_type'] = 'pressButton'
...
subjects = [str(i).zfill(3) for i in range(79,83)]
...
if task == 'game1':
    ...
    template = {'behav_path':r'/mnt/workdir/DCM/sourcedata/sub_{}/Behaviour/fmri_task-game1/sub-{}_task-{}_run-{}.csv',
                'save_dir':r'/mnt/workdir/DCM/BIDS/derivatives/Events/sub-{}/{}/M2_Decision/{}fold',
