...
#%%
task = 'game1'  # look out
glm_type = 'separate_hexagon_correct_trials_train'
set_id = 'Set2'
...
# covariate
level2nd_covar_acc(data,task,glm_type,set_id,contrast_1st)
level2nd_covar_age(data,task,glm_type,set_id,contrast_1st)
#level2nd_covar_acc_age(data,task,glm_type, contrast_1st)

# all subjects
pid = data['Participant_ID'].to_list()
sub_list = [p.split('-')[-1] for p in pid]
level2nd_noPhi(sub_list,'all',task,glm_type, set_id, contrast_1st)
