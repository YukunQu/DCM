...
data = data.query('Participant_ID!="sub-187"')  # look out
...
hp_data = data.query("(game1_acc>=0.7)")  # look out
hp_pid = hp_data['Participant_ID'].to_list()
hp_sub_list = [p.split('-')[-1] for p in hp_pid]
out_container = f'{task}/{glm_type}/{set_id}/{ifold}/group/acc_0.7'  # output = data_root + out_container
level2nd_onesample_ttest(hp_sub_list,contrast_1st,data_root,templates,out_container)
