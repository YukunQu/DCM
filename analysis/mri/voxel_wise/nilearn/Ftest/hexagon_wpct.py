...
def load_ev(event_path):
    ...
    event_condition = event.query("trial_type in ['M1','infer_corr','infer_error','cos','sin']")
    event_condition = event_condition[['onset', 'duration', 'trial_type', 'modulation']]
    return event_condition


def set_contrasts(design_matrix):
    contrast_name = ['M1', 'infer_corr', 'infer_error', 'sin','cos']
    # base contrast
    contrasts_set = {}
    ...
    contrasts_set['hexagon'] = np.vstack([contrasts_set['cos'],
                                          contrasts_set['sin']])
    if 'infer_error' in contrasts_set.keys():
        contrasts_set['correct_error'] = contrasts_set['infer_corr'] - contrasts_set['infer_error']
...
def run_glm(task,subj):
    if task == 'game1':
        ifold = 6
        configs = {'TR': 3.0, 'task': 'game1', 'glm_type': 'hexagon_wpct',
        ...
        ifold = 6
        configs = {'TR': 3.0, 'task': 'game2', 'glm_type': 'hexagon_wpct',
                   'run_list': [1, 2],
                   'func_dir': r'/mnt/workdir/DCM/BIDS/derivatives/fmriprep_volume_fmapless/fmriprep',
                   'event_dir': r'/mnt/workdir/DCM/BIDS/derivatives/Events',
...
if __name__ == "__main__":
    ...
    for chunk in subjects_chunk:
        results_list = Parallel(n_jobs=5)(delayed(run_glm)(task,subj) for subj in chunk)