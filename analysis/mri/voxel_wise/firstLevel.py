...
def firstLevel_noPhi(subject_list,set_id,runs,ifold,configs):
    ...
    elif glm_type == 'separate_hexagon_old':
        firstLevel_noPhi_separate(subject_list,set_id,runs,ifold,configs)
    elif glm_type == 'separate_hexagon_new':
        firstLevel_noPhi_separate_new(subject_list,set_id,runs,ifold,configs)
    elif glm_type == 'separate_hexagon_difficult':
        firstLevel_noPhi_difficult(subject_list,set_id,runs,ifold,configs)
    elif glm_type == 'whole_hexagon':
        firstLevel_noPhi_whole(subject_list,set_id,runs,ifold,configs)
    elif glm_type == 'RSA':
...
def run_info_separate(ev_file,motions_file=None):
    ...
    run_info = Bunch(conditions=conditions,onsets=onsets,durations=durations,
                     pmod=[None,run_pmod,None,run_pmod,None,None],
...
def run_info_separate_new(ev_file,motions_file=None):
    import pandas as pd
    from nipype.interfaces.base import Bunch
    onsets = []
    conditions = []
    ...
    ev_info = pd.read_csv(ev_file, sep='\t')
    trial_con = ['M1','M2','decision','pressButton']
    for group in ev_info.groupby('trial_type'):
        condition = group[0]
        if condition in trial_con:
            conditions.append(condition)
    ...
    run_pmod = Bunch(name=pmod_names,param=pmod_params,poly=pmod_polys)
    run_info = Bunch(conditions=conditions,onsets=onsets,durations=durations,
                     pmod=[None,run_pmod,run_pmod,None],
                     orth=['No','No','No','No'],regressor_names=motion_columns,regressors=motions)
    return run_info


def firstLevel_noPhi_separate_new(subject_list,set_id,runs,ifold,configs):
    ...
    runs_prep = MapNode(Function(input_names=['ev_file','motions_file'],
                                 output_names=['run_info'],
                                 function=run_info_separate_new),
                        name='runsinfo',
                        iterfield=['ev_file','motions_file'])
...
def run_info_whole(ev_file,motions_file=None):
    import pandas as pd
    from nipype.interfaces.base import Bunch
    onsets = []
    conditions = []
...
def firstLevel_noPhi_whole(subject_list,set_id,runs,ifold,configs):
    start_time = time.time()
    print("Training set",set_id," ",ifold," start!")
