...
def run_firstLevel_spm(subject_list, set_id, runs, ifold, configs):
    glm_type = configs['glm_type']
    if glm_type == 'hexagon_spct':
        firstLevel_hexagon_spct(subject_list, set_id, runs, ifold, configs)
    elif glm_type == 'hexagon_spat':
        firstLevel_hexagon_spat(subject_list, set_id, runs, ifold, configs)
    elif glm_type == 'distance_spct':
        firstLevel_distance_spct(subject_list, set_id, runs, ifold, configs)
    elif glm_type == 'distance_spat':
        firstLevel_distance_spat(subject_list, set_id, runs, ifold, configs)
    elif glm_type == 'distance_m2':
        firstLevel_distance_m2(subject_list, set_id, runs, ifold, configs)
    elif glm_type == 'distance_decision':
        firstLevel_distance_decision(subject_list, set_id, runs, ifold, configs)
    ...
    elif glm_type == 'grid_rsa_corr_trials':
        firstLevel_grid_rsa(subject_list, set_id, runs, ifold, configs)
    elif glm_type == 'decision_grid_rsa':
        firstLevel_grid_rsa(subject_list, set_id, runs, ifold, configs)
...
def firstLevel_workflow(subject_list, set_id, runs, ifold, configs, run_info, contrast_list):
    """
    :param subject_list: the list of sub_id,using number string
    :param set_id:
    :param runs: run list specify which runs will be used.
    :param ifold: specify ifold
    :param configs: specify parameters
    :param run_info: run_info function to prepare event data
    :param contrast_list: set contrast
    """
    # start cue
    ...
    # Specify Workflow
    # Initiation of the 1st-level analysis workflow and set working directory
    working_dir = f'{nipype_dir}/working_dir/{task}/{glm_type}/Set{set_id}/{ifold}'
    ...
    # EstimateContrast - Specify GLM contrasts and estimates contrasts
    ...
    datasink = Node(DataSink(base_directory=nipype_dir, container=container_path),
                    name="datasink")
    ...
    analysis1st.run('MultiProc', {'n_procs': 10, 'memory_gb': 30})
    end_time = time.time()
    run_time = round((end_time - start_time) / 60 / 60, 2)
    print(f"Run time cost {run_time}")


def run_info_hexagon_spct(ev_file, motions_file=None):
    import pandas as pd
...
def firstLevel_hexagon_spct(subject_list, set_id, runs, ifold, configs):
    ...
    cont08 = ['m2_error', 'T', condition_names, [0, 0, 0, 0, 0, 1, 0, 0]]
    ...
    cont012 = ['m2_correctVSerror', 'T', condition_names, [0, 0, 0, 0, 1, -1, 0, 0]]

    cont013 = ['decision_corr', 'T', condition_names, [0, 0, 0, 0, 0, 0, 1, 0]]
    cont014 = ['decision_error', 'T', condition_names, [0, 0, 0, 0, 0, 0, 0, 1]]
    cont015 = ['decision_correctVSerror', 'T', condition_names, [0, 0, 0, 0, 0, 0, 1, -1]]
    ...
    firstLevel_workflow(subject_list, set_id, runs, ifold, configs, run_info_hexagon_spct, contrast_list)


def run_info_hexagon_spat(ev_file, motions_file=None):
    import pandas as pd
    from nipype.interfaces.base import Bunch
    onsets = []
    conditions = []
...
def firstLevel_hexagon_spat(subject_list, set_id, runs, ifold, configs):
    # Specify GLM contrasts
    # Condition names
    ...
    firstLevel_workflow(subject_list, set_id, runs, ifold, configs,run_info_hexagon_spat,contrast_list)


def run_info_distance_spct(ev_file, motions_file=None):
    import pandas as pd
    from nipype.interfaces.base import Bunch
    onsets = []
    conditions = []
...
def firstLevel_distance_spct(subject_list, set_id, runs, ifold, configs):
    # Specify GLM contrasts
    # Condition names
    condition_names = ['M2_corrxdistance^1', 'decision_corrxdistance^1','M2_corr', 'M2_error','decision_corr','decision_error']

    # contrastst
    cont01 = ['m2_distance', 'T', condition_names, [1, 0, 0, 0, 0, 0]]
    cont02 = ['decision_distance', 'T', condition_names, [0, 1, 0, 0, 0, 0]]
    cont03 = ['m2_corr',       'T', condition_names, [0, 0, 1, 0, 0, 0]]
    cont04 = ['decision_corr', 'T', condition_names, [0, 0, 0, 0, 1, 0]]
    cont05 = ['distance', 'T', condition_names, [1, 1, 0, 0, 0, 0]]
    cont06 = ['decision_corrVSerror', 'T', condition_names, [0, 0, 0, 0, 1, -1]]

    contrast_list = [cont01, cont02, cont03, cont04, cont05,cont06]
    firstLevel_workflow(subject_list, set_id, runs, ifold, configs, run_info_distance_spct,contrast_list)


def run_info_distance_spat(ev_file, motions_file=None):
    import pandas as pd
    from nipype.interfaces.base import Bunch
    onsets = []
    conditions = []
...
def firstLevel_distance_spat(subject_list, set_id, runs, ifold, configs):
    # Specify GLM contrasts
    # Condition names
    condition_names = ['M2xdistance^1', 'decisionxdistance^1','M2', 'decision']
    ...
    firstLevel_workflow(subject_list, set_id, runs, ifold, configs,run_info_distance_spat,contrast_list)


def run_info_distance_m2(ev_file, motions_file=None):
    import pandas as pd
    from nipype.interfaces.base import Bunch
    onsets = []
    conditions = []
...
def firstLevel_distance_m2(subject_list, set_id, runs, ifold, configs):
    # start cue
    start_time = time.time()
    print("Training set", set_id, " ", ifold, " start!")
    ...
    # Specify GLM contrasts
    # Condition names
    condition_names = ['M2_corrxdistance^1', 'M2_corr', 'decision_corr', 'decision_error']

    # contrastst
    cont01 = ['distance', 'T', condition_names, [1, 0, 0, 0]]
    cont02 = ['m2', 'T', condition_names, [0, 1, 0, 0]]
    cont03 = ['decision_corr', 'T', condition_names, [0, 0, 1, 0]]
    cont04 = ['decision_error', 'T', condition_names, [0, 0, 0, 1]]
    cont05 = ['decision_contrast', 'T', condition_names, [0, 0, 1, -1]]
...
def run_info_distance_decision(ev_file, motions_file=None):
    import pandas as pd
    from nipype.interfaces.base import Bunch
    onsets = []
    conditions = []
    ...
    run_pmod = Bunch(name=pmod_names, param=pmod_params, poly=pmod_polys)
    if conditions == ['M1', 'M2_corr', 'M2_error', 'decision_corr', 'decision_error']:
        pmod = [None, None, None, run_pmod, None]
        orth = ['No', 'No', 'No', 'No', 'No']
    elif conditions == ['M1', 'M2_corr', 'decision_corr']:
...
def firstLevel_distance_decision(subject_list, set_id, runs, ifold, configs):
    # start cue
    start_time = time.time()
    print("Training set", set_id, " ", ifold, " start!")
    ...
    # Specify GLM contrasts
    # Condition names
    condition_names = ['decision_corrxdistance^1', 'M2_corr', 'decision_corr', 'decision_error']
    ...
    runs_prep = MapNode(Function(input_names=['ev_file', 'motions_file'],
                                 output_names=['run_info'],
                                 function=run_info_distance_decision),
                        name='runsinfo',
                        iterfield=['ev_file', 'motions_file'])
