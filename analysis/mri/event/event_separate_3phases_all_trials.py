...
class Game1EV(object):
    ...
    def game1ev(self,ifold):
        ...
        m1ev = self.genM1ev()
        m2ev = self.genM2ev()
        planning = self.genPlanning()
        pbev = self.pressButton()
        deev = self.genDeev()
        pmod_sin, pmod_cos = self.genpm(m2ev,ifold)

        event_data = pd.concat([m1ev,m2ev,planning,deev,pbev,
...
def gen_sub_event(task, subjects):
    if task == 'game1':
        ...
        template = {'behav_path':r'/mnt/workdir/DCM/sourcedata/sub_{}/Behaviour/fmri_task-game1/sub-{}_task-{}_run-{}.csv',
                    'save_dir':r'/mnt/workdir/DCM/BIDS/derivatives/Events/{}/separate_3phases/sub-{}/{}fold',
