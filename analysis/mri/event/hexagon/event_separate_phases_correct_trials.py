    ...
    for subj in subjects:
        ...
        for ifold in ifolds:
            ...
            for idx in runs:
                ...
                if task_type == 'game1':
                    event = Game1EV_spct(behDataPath)
                    event_data = event.game1ev_spct(ifold)
                elif task_type == 'game2':
                    event = Game2EV_spct(behDataPath)
                    event_data = event.game2ev_spct(ifold)
                else:
                    raise Exception("The type of task is wrong.")
...
if __name__ == "__main__":
    ...
    subjects_list = [s for s in subjects_list if int(s) > 237]
